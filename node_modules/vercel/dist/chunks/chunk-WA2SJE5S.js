...
function asPackageManager(cliType) {
  if (cliType === "npm" || cliType === "pnpm" || cliType === "yarn" || cliType === "bun" || cliType === "vlt") {
    return cliType;
  }
  return "npm";
}
...
async function getUpdateCommandInfo() {
  ...
  if (nativeInstall) {
    ...
    let packageManager2 = "npm";
    if (segments.includes("pnpm") || segments.includes(".pnpm")) {
      packageManager2 = "pnpm";
    } else if (segments.includes("yarn") || segments.includes(".yarn")) {
      packageManager2 = "yarn";
    }
    const install2 = packageManager2 === "yarn" ? "global add" : "i -g";
    const force = packageManager2 === "npm" ? " --force" : "";
    const allowBuild2 = pnpmAllowBuildFlag(packageManager2, nativePackageName);
    return {
      command: `${packageManager2} ${install2} ${pkgAndVersion}${force}${allowBuild2}`,
      global: true,
      packageManager: packageManager2,
      // Native path detection only positively identifies pnpm/yarn.
      assumed: packageManager2 === "npm"
    };
  }
  ...
  const allowBuild = global ? pnpmAllowBuildFlag(packageManager, "esbuild") : "";
  return {
