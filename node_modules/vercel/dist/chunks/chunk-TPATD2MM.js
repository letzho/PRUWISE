...
// ../../internals/builder-orchestration/dist/merge.js
var require_merge = __commonJS({
  "../../internals/builder-orchestration/dist/merge.js"(exports, module) {
    "use strict";
    ...
    var import_fs_extra = require_lib();
    var mergeQueue = Promise.resolve();
    function merge(source, destination, ignoreFilter, sourceRoot) {
    ...
    async function mergeImpl(source, destination, ignoreFilter, sourceRoot) {
      const root = sourceRoot || source;
      if (ignoreFilter) {
        const relPath = (0, import_node_path.relative)(root, source);
        if (relPath && !ignoreFilter(relPath)) {
          await (0, import_fs_extra.remove)(source);
          return;
        }
      }
      const destStat = await (0, import_fs_extra.stat)(destination).catch(
        (err) => err
      );
      if ((0, import_error_utils.isErrnoException)(destStat)) {
        ...
        const contents = await (0, import_fs_extra.readdir)(
          source
        ).catch((err) => err);
        if ((0, import_error_utils.isErrnoException)(contents)) {
          ...
          await Promise.all(
            contents.map(
              (name) => mergeImpl(
                (0, import_node_path.join)(source, name),
                (0, import_node_path.join)(destination, name),
                ignoreFilter,
                root
              )
            )
          );
          await (0, import_fs_extra.rmdir)(source);
          return;
        }
      }
      await overwrite(source, destination, ignoreFilter, root);
    }
    async function publishNew(source, destination, ignoreFilter, root) {
      let renameError;
      try {
        try {
          ...
          if ((0, import_error_utils.isErrnoException)(err) && err.code === "ENOENT") {
            await (0, import_fs_extra.mkdirp)((0, import_node_path.dirname)(destination));
            await (0, import_promises.rename)(source, destination);
            return;
          }
          throw err;
      ...
      if ((0, import_error_utils.isErrnoException)(renameError) && renameError.code === "EXDEV") {
        const tmp = tmpName(destination);
        try {
          await (0, import_fs_extra.move)(source, tmp);
        } catch (err) {
          await (0, import_fs_extra.remove)(tmp).catch(() => void 0);
          throw err;
        }
        try {
          await (0, import_promises.rename)(tmp, destination);
        } catch {
          const relPrefix = (0, import_node_path.relative)(root, source);
          try {
            await mergeImpl(
              tmp,
              destination,
              ignoreFilter && ((path) => ignoreFilter((0, import_node_path.join)(relPrefix, path))),
              tmp
            );
          } catch (err) {
            await (0, import_fs_extra.remove)(tmp).catch(() => void 0);
      ...
      const destinationAppeared = await (0, import_fs_extra.stat)(destination).then(
        () => true,
        () => false
      );
    ...
    async function overwrite(source, destination, ignoreFilter, root) {
      const tmp = tmpName(destination);
      try {
        await (0, import_fs_extra.move)(source, tmp);
      } catch (err) {
      ...
      try {
        await (0, import_fs_extra.remove)(destination);
        await (0, import_promises.rename)(tmp, destination);
      } catch {
        const relPrefix = (0, import_node_path.relative)(root, source);
        try {
    ...
    function tmpName(destination) {
      return `${destination}.tmp-${process.pid}-${Math.random().toString(36).slice(2)}`;
    }
  }
});
...
// ../../internals/builder-orchestration/dist/unzip.js
var require_unzip = __commonJS({
  "../../internals/builder-orchestration/dist/unzip.js"(exports, module) {
    "use strict";
    ...
    var import_promisepipe = __toESM2(require_promisepipe());
    var fs = __toESM2(require_lib());
    var import_build_utils = __require("@vercel/build-utils");
    var import_yauzl_promise = require_lib3();
    ...
    async function unzip(buffer, dir) {
      ...
      for await (const entry of createZipIterator(zipFile)) {
        if (entry.fileName.startsWith("__MACOSX/"))
          continue;
        try {
          const destDir = import_path.default.dirname(import_path.default.join(dir, entry.fileName));
          await fs.mkdirp(destDir);
          const canonicalDestDir = await fs.realpath(destDir);
          const relativeDestDir = import_path.default.relative(dir, canonicalDestDir);
          if (relativeDestDir.split(import_path.default.sep).includes("..")) {
            throw new Error(
    ...
    async function extractEntry(zipFile, entry, dir) {
      const dest = import_path.default.join(dir, entry.fileName);
      ...
      const destDir = isDir ? dest : import_path.default.dirname(dest);
      ...
      if (symlink) {
        const link = await (0, import_build_utils.streamToBuffer)(readStream);
        await fs.symlink(link.toString("utf8"), dest);
      } else {
        await (0, import_promisepipe.default)(readStream, fs.createWriteStream(dest, { mode: procMode }));
      }
    }
...
// ../../internals/builder-orchestration/dist/write-build-result.js
var require_write_build_result = __commonJS({
  "../../internals/builder-orchestration/dist/write-build-result.js"(exports, module) {
    "use strict";
    var __create = Object.create;
    var __defProp = Object.defineProperty;
    var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
    ...
    var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);
    var write_build_result_exports = {};
    __export(write_build_result_exports, {
      OUTPUT_DIR: () => OUTPUT_DIR2,
      createWriteBuildResult: () => createWriteBuildResult2,
      filesWithoutFsRefs: () => filesWithoutFsRefs2,
      findDirs: () => findDirs2,
      isLambda: () => isLambda2,
      relocateRootBuildOutputToService: () => relocateRootBuildOutputToService2
    });
    module.exports = __toCommonJS(write_build_result_exports);
    ...
    var import_unzip = require_unzip();
    var import_client = require_dist2();
    var { normalize } = import_path.posix;
    var OUTPUT_DIR2 = (0, import_path.join)(".vercel", "output");
    function createWriteBuildResult2(output) {
      return {
        writeBuildResult: (args) => writeBuildResult2(args, output),
        relocateRootBuildOutputToService: relocateRootBuildOutputToService2,
        filesWithoutFsRefs: filesWithoutFsRefs2
      };
    }
    ...
    async function writeBuildResult2(args, outputManager) {
      const {
        repoRootPath,
        outputDir,
        buildResult,
    ...
    async function resolveFunctionConfiguration(src, vercelConfig, service) {
      if (service && (0, import_build_utils.isExperimentalServiceV2)(service) && service.functions) {
        ...
        return (0, import_build_utils.getLambdaOptionsFromFunction)({
          sourceFile,
          config: {
            ...vercelConfig,
    ...
    function getServiceOutputDir(outputDir, service) {
      return (0, import_path.join)(outputDir, "services", service.name);
    }
    async function relocateRootBuildOutputToService2(args) {
    ...
    async function writeBuildResultV2(args) {
      ...
      if (!buildResult.output) {
        const configFile = vercelConfig?.[import_client.fileNameSymbol];
        const updateMessage = build.use.startsWith("@now/") ? ` Please update from "@now" to "@vercel" in your \`${configFile}\` file.` : "";
        throw new Error(
      ...
      const overrides = {};
      const functionConfiguration = build.src ? await resolveFunctionConfiguration(build.src, vercelConfig, service) : {};
      for (const [path, output] of Object.entries(buildResult.output)) {
        const normalizedPath = stripDuplicateSlashes(path);
        if (isContainerImage(output)) {
          injectServiceEnvVars(
            output,
            service && (0, import_build_utils.isExperimentalService)(service) ? service : void 0,
            stripServiceRoutePrefix
          );
          ...
          if (fallback) {
            ...
            const fallbackPath = (0, import_path.join)(outputDir, "functions", fallbackName);
            let usedHardLink = false;
            if ("fsPath" in fallback) {
              try {
                await import_fs_extra.default.link(fallback.fsPath, fallbackPath);
                usedHardLink = true;
              } catch (_) {
              }
            }
            if (!usedHardLink) {
              ...
              await (0, import_promisepipe.default)(
                stream,
                import_fs_extra.default.createWriteStream(fallbackPath, { mode: fallback.mode })
              );
            }
            fallback = new import_build_utils.FileFsRef({
              ...output.fallback,
              fsPath: (0, import_path.basename)(fallbackName)
            });
          }
          ...
          await import_fs_extra.default.writeJSON(prerenderConfigPath, prerenderConfig, { spaces: 2 });
    ...
    async function writeBuildResultV3(args) {
      ...
      const routesJsonPath = (0, import_path.join)(workPath, ".vercel", "routes.json");
      if ((0, import_build_utils.isBackendBuilder)(build) || build.use === "@vercel/python") {
        if ((0, import_fs_extra.existsSync)(routesJsonPath)) {
          try {
            const newOutput = {
              index: output
            };
            const routesJson = await import_fs_extra.default.readJSON(routesJsonPath);
        ...
        if ((0, import_build_utils.isBackendBuilder)(build) && (0, import_build_utils.isExperimentalBackendsEnabled)() && "routes" in buildResult) {
          return writeBuildResultV2({
            repoRootPath,
            outputDir,
            buildResult,
      ...
      const ext = (0, import_path.extname)(src);
      const path = service && (0, import_build_utils.isExperimentalServiceV2)(service) ? "index" : service && typeof service.runtime === "string" ? stripDuplicateSlashes((0, import_build_utils.getInternalServiceFunctionPath)(service.name)) : stripDuplicateSlashes(
    ...
    async function writeStaticFile(outputDir, file, path, overrides, cleanUrls = false) {
      let fsPath = path;
      let override = null;
      const ext = getFileExtension(file);
      if (ext && (0, import_path.extname)(path) !== ext) {
        fsPath += ext;
        if (!override)
          override = {};
        override.path = path;
      ...
      const dest = (0, import_path.join)(outputDir, "static", fsPath);
      await import_fs_extra.default.mkdirp((0, import_path.dirname)(dest));
      if ("fsPath" in file) {
        if ((0, import_path.resolve)(file.fsPath) === (0, import_path.resolve)(dest)) {
          return;
        }
        try {
          return await import_fs_extra.default.link(file.fsPath, dest);
        } catch (_) {
        }
      }
      await (0, import_build_utils.downloadFile)(file, dest);
    }
    async function writeFunctionSymlink(outputDir, dest, fn, existingFunctions) {
      const existingPath = existingFunctions.get(fn);
      if (!existingPath)
        return false;
      const destDir = (0, import_path.dirname)(dest);
      const targetDest = (0, import_path.join)(outputDir, "functions", `${existingPath}.func`);
      const target = (0, import_path.relative)(destDir, targetDest);
      await import_fs_extra.default.mkdirp(destDir);
      await import_fs_extra.default.symlink(target, dest);
      return true;
    }
    async function writeContainerImage(outputDir, containerImage, path, functionConfiguration) {
      const dest = (0, import_path.join)(outputDir, "functions", `${path}.func`);
      ...
      await import_fs_extra.default.mkdirp(dest);
    ...
    async function writeEdgeFunction(repoRootPath, outputDir, edgeFunction, path, existingFunctions, standalone = false) {
      const dest = (0, import_path.join)(outputDir, "functions", `${path}.func`);
      ...
      await import_fs_extra.default.mkdirp(dest);
      const ops = [];
      const { files, filePathMap } = filesWithoutFsRefs2(
        edgeFunction.files,
        repoRootPath,
        standalone
      );
      ops.push((0, import_build_utils.download)(files, dest));
      const config = {
        runtime: "edge",
        ...edgeFunction,
        entrypoint: (0, import_build_utils.normalizePath)(edgeFunction.entrypoint),
        filePathMap,
        files: void 0,
        type: void 0
      };
      const configPath = (0, import_path.join)(dest, ".vc-config.json");
      ops.push(
        import_fs_extra.default.writeJSON(configPath, config, {
          spaces: 2
        })
      );
      await Promise.all(ops);
    }
    async function writeLambda(repoRootPath, outputDir, lambda, path, functionConfiguration, existingFunctions, standalone = false) {
      const dest = (0, import_path.join)(outputDir, "functions", `${path}.func`);
      if (existingFunctions) {
      ...
      let filePathMap;
      if (lambda.files) {
        const f = filesWithoutFsRefs2(lambda.files, repoRootPath, standalone);
        filePathMap = f.filePathMap;
        ops.push((0, import_build_utils.download)(f.files, dest));
      } else if (lambda.zipBuffer) {
        ops.push((0, import_unzip.unzip)(lambda.zipBuffer, dest));
      } else {
      ...
      const config = {
        ...lambda,
        handler: (0, import_build_utils.normalizePath)(lambda.handler),
        architecture,
        memory,
        maxDuration,
        affinity,
      ...
      await Promise.all(ops);
      for await (const dir of findDirs2(".vercel", dest)) {
        const absDir = (0, import_path.join)(dest, dir);
        const entries = await import_fs_extra.default.readdir(absDir);
        if (entries.includes("cache")) {
          await Promise.all(
            entries.filter((e) => e !== "cache").map((entry) => import_fs_extra.default.remove((0, import_path.join)(absDir, entry)))
          );
        } else {
          await import_fs_extra.default.remove(absDir);
        }
      }
    }
    async function mergeBuilderOutput(outputDir, buildResult, workPath, rootOutputDir, outputManager) {
      const absOutputDir = (0, import_path.resolve)(outputDir);
      const absRootOutputDir = (0, import_path.resolve)(rootOutputDir);
      const { ig } = await (0, import_client.getVercelIgnore)(workPath);
      ...
      if ((0, import_path.resolve)(buildResult.buildOutputPath) === absRootOutputDir) {
      ...
      await (0, import_merge.merge)(buildResult.buildOutputPath, outputDir, ignoreFilter);
    }
    async function relocateBuildOutputApiEntries(rootOutputDir, outputDir, workPath) {
      const { ig } = await (0, import_client.getVercelIgnore)(workPath);
      const filter = ig.createFilter();
      await import_fs_extra.default.mkdirp(outputDir);
      await Promise.all(
        BUILD_OUTPUT_API_ENTRIES.map(async (entry) => {
          const src = (0, import_path.join)(rootOutputDir, entry);
          if (!await import_fs_extra.default.pathExists(src))
            return;
          const dest = (0, import_path.join)(outputDir, entry);
          if (entry === "static") {
            await (0, import_merge.merge)(src, dest, (path) => filter(path));
          } else {
            await (0, import_merge.merge)(src, dest);
          }
        })
      );
    }
    async function cleanIgnoredFiles(dir, staticRoot, filter, outputManager) {
      ...
      await Promise.all(
        entries.map(async (entry) => {
          ...
          const relativePath = (0, import_path.relative)(staticRoot, entryPath);
          if (stat.isDirectory()) {
            await cleanIgnoredFiles(entryPath, staticRoot, filter, outputManager);
            const remaining = await import_fs_extra.default.readdir(entryPath);
            if (remaining.length === 0) {
              await import_fs_extra.default.rmdir(entryPath);
            }
    ...
    function getFileExtension(file) {
      ...
      if (file.type === "FileFsRef") {
        ext = (0, import_path.extname)(file.fsPath);
      }
    ...
    async function* findDirs2(name, dir, root = dir) {
      let paths;
      try {
        paths = await import_fs_extra.default.readdir(dir);
      } catch (err) {
        if (err.code !== "ENOENT") {
          throw err;
        }
      ...
      for (const path of paths) {
        ...
        let stat;
        try {
          stat = await import_fs_extra.default.lstat(abs);
        } catch (err) {
          if (err.code === "ENOENT")
            continue;
          throw err;
        ...
        if (stat.isDirectory()) {
          if (path === name) {
            yield (0, import_path.relative)(root, abs);
          } else {
            yield* findDirs2(name, abs, root);
          }
        }
      }
    }
    function filesWithoutFsRefs2(files, repoRootPath, standalone) {
      const symlinkDirs = standalone ? Object.entries(files).filter(([, f]) => f.type === "FileFsRef" && (0, import_build_utils.isSymbolicLink)(f.mode)).map(([p]) => `${(0, import_build_utils.normalizePath)(p)}/`) : [];
      let filePathMap;
      const out = {};
      for (const [path, file] of Object.entries(files)) {
        if (file.type === "FileFsRef") {
          if (standalone) {
            const normalized = (0, import_build_utils.normalizePath)(path);
            ...
            if (!filePathMap)
              filePathMap = {};
            filePathMap[(0, import_build_utils.normalizePath)(path)] = (0, import_build_utils.normalizePath)(
              (0, import_path.relative)(repoRootPath, file.fsPath)
            );
          }
        } else {
          out[path] = file;
...
// ../../internals/cli-builder-integration/dist/get-services-builders.js
var require_get_services_builders = __commonJS({
  "../../internals/cli-builder-integration/dist/get-services-builders.js"(exports, module) {
    "use strict";
    var __defProp = Object.defineProperty;
    var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
    var __getOwnPropNames = Object.getOwnPropertyNames;
    ...
    async function getServicesBuilders(options) {
      ...
      const fs = new import_fs_detectors.LocalFileSystemDetector(workPath);
...
// ../../internals/cli-builder-integration/dist/detect-builders-with-services.js
var require_detect_builders_with_services = __commonJS({
  "../../internals/cli-builder-integration/dist/detect-builders-with-services.js"(exports, module) {
    "use strict";
    var __defProp = Object.defineProperty;
    var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
    var __getOwnPropNames = Object.getOwnPropertyNames;
    ...
    var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);
    var detect_builders_with_services_exports = {};
    __export(detect_builders_with_services_exports, {
      detectBuildersWithServices: () => detectBuildersWithServices
    });
    module.exports = __toCommonJS(detect_builders_with_services_exports);
    var import_fs_detectors = require_dist3();
    var import_get_services_builders = require_get_services_builders();
    async function detectBuildersWithServices(files, pkg, options = {}) {
      const {
      ...
      const proxyError = (0, import_fs_detectors.validateProxy)(options, files, projectSettings.framework);
      if (proxyError) {
        return {
          builders: null,
          errors: [proxyError],
      ...
      if (!hasServicesConfig && projectSettings.framework !== "services") {
        return (0, import_fs_detectors.detectBuilders)(files, pkg, options);
      }
      ...
      if (configuredServices != null) {
        result.warnings.push(...(0, import_get_services_builders.warnIgnoredDirectories)(files, configuredServices));
      }
      if (!result.errors && options.proxy) {
        result.builders = [
          (0, import_fs_detectors.getProxyBuilder)(options.proxy, options.tag, options.functions),
          ...result.builders ?? []
        ];
      }
      return result;
...
// ../../internals/cli-builder-integration/dist/service-schedules.js
var require_service_schedules = __commonJS({
  "../../internals/cli-builder-integration/dist/service-schedules.js"(exports, module) {
    "use strict";
    var __defProp = Object.defineProperty;
    var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
    var __getOwnPropNames = Object.getOwnPropertyNames;
    ...
    var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);
    var service_schedules_exports = {};
    __export(service_schedules_exports, {
      getStaticServiceSchedules: () => getStaticServiceSchedules
    });
    module.exports = __toCommonJS(service_schedules_exports);
...
// ../../internals/cli-builder-integration/dist/get-files.js
var require_get_files = __commonJS({
  "../../internals/cli-builder-integration/dist/get-files.js"(exports, module) {
    "use strict";
    var __create = Object.create;
    var __defProp = Object.defineProperty;
    var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
    ...
    var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);
    var get_files_exports = {};
    __export(get_files_exports, {
      staticFiles: () => staticFiles
    });
    module.exports = __toCommonJS(get_files_exports);
    var import_fs_extra = __toESM2(require_lib());
    var import_path = __require("path");
    var import_client = require_dist2();
    ...
    var asAbsolute = function(path, parent) {
      if (path[0] === "/") {
        return path;
      }
      return (0, import_path.resolve)(parent, path);
    };
    async function staticFiles(path, { src }, output) {
      const { debug, time } = output;
      let files = [];
      const source = src || ".";
      const search = (0, import_path.resolve)(path, source);
      const { ig } = await (0, import_client.getVercelIgnore)(path);
      const filter = ig.createFilter();
    ...
    async function explode(paths, { accepts, output }) {
      ...
      const list = async (file) => {
        ...
        try {
          s = await import_fs_extra.default.stat(path);
        } catch (_e) {
          path = `${file}.js`;
          try {
            s = await import_fs_extra.default.stat(path);
...
// src/builders/write-build-result.ts
var import_write_build_result = __toESM(require_write_build_result(), 1);
var {
  filesWithoutFsRefs,
  relocateRootBuildOutputToService,
  writeBuildResult
} = (0, import_write_build_result.createWriteBuildResult)(output_manager_default);
