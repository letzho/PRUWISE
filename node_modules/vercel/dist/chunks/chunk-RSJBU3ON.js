...
import {
  progress
} from "./chunk-2EAIC3SH.js";
import {
  isJSONObject,
  printIndications,
  require_dist as require_dist2,
  sleep
} from "./chunk-Q5K5DEOK.js";
import {
  suggestNextCommands
} from "./chunk-7ST552E3.js";
import {
  CommandTimeout
} from "./chunk-4KVJ2M4J.js";
import {
  stamp_default
...
async function processDeployment({
  ...
  const deployingSpinnerVal = isSettingUpProject ? "Setting up project" : `Deploying ${import_chalk2.default.bold(
  ...
  const linkedProjectLookup = linkedProjectIsPartial && linkedProject && !noWait ? getProjectByNameOrId(
    client,
    linkedProject.id,
  ...
  let project = linkedProjectLookup ? void 0 : linkedProject;
  ...
  try {
    for await (const event of (0, import_client.createDeployment)(clientOptions, requestBody)) {
      ...
      if (project === void 0 && (!linkedProjectLookup || event.type === "ready")) {
      ...
      if (event.type === "alias-assigned") {
        ...
        if (event.payload.target === "production" && event.payload.alias && event.payload.alias.length > 0) {
          ...
          printAlignedLabel(
            ...
            import_chalk2.default.cyan(prodUrl),
            {
              gutter: "\u25B2"
            }
          );
          if (anonymous && !jsonOutput && (quiet || process.env.FORCE_TTY === "1")) {
...
async function printDeploymentStatus(client, {
  ...
  url,
  target
}, deployStamp, noWait, guidanceMode, isInit, gitConnectCommand) {
...
`
  ...
  if (gitConnectCommand || guidanceMode) {
    ...
    if (gitConnectCommand) {
      nextCommands.push({
        description: "Automatically deploy changes on every push by connecting Git",
...
async function performAddRequest(client, domainName) {
  return (0, import_async_retry4.default)(
    async () => {
      try {
        const { domain } = await client.fetch("/v7/domains", {
