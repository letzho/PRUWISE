/* =============================================================================
   scripts/create-200-complete.mjs  -  Create exactly 200 complete clients
   -----------------------------------------------------------------------------
   - Deletes all generated clients
   - Keeps original demos (Sarah, etc.)
   - Creates new clients to reach exactly 200 total
   - Each has: unique name, contact, completed assessment, random financial data
...
function generateClient(index, usedNames) {
    ...
    const existingPA = hasSomeCover && Math.random() < 0.25 ? randomAmount(10000, 150000, 10000) : 0;
    ...
    // Health conditions more common with age
    const healthConditions = Math.random() < (age - 20) / 120;
...
async function main() {
    console.log('\n✨ Creating 200 Complete Clients with Assessments\n');
    ...
    try {
        console.log('Starting client generation loop...');
        for (let i = 1; i <= needed; i++) {
            ...
            clientSince.setDate(clientSince.getDate() - randomInt(1, 730));
            ...
            await sql`INSERT INTO people (id, kind, name, first_name, email, phone, rep_id, client_since)
                ...
                ${client.phone}, ${kristinId}, ${clientSince.toISOString().split('T')[0]})`;
            ...
            // Create realistic assessment based on their profile
            const answers = {
